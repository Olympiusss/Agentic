...
Sentry Agentic integrates with four sandboxes:
...
- **Hash-only**: Sentry Agentic never uploads binary bytes. Submission is via hash
...
- **API-only transport**: no shared filesystems between Sentry Agentic and the
