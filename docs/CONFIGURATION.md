...
Sentry Agentic splits configuration into two stores:
...
2. **The encrypted secret store** at `~/.sentry-agentic/secrets.enc` — every API key,
   ...
   master key sits next to it at `~/.sentry-agentic/master.key`.
...
1. Encrypted file (`~/.sentry-agentic/secrets.enc`)
...
```bash
docker run --env-file .env sentry-agentic
```
...
```ini
[Service]
EnvironmentFile=/opt/sentry-agentic/.env
ExecStart=/opt/sentry-agentic/venv/bin/uvicorn backend.main:app --host 0.0.0.0 --port 6987
```
...
```bash
kubectl create secret generic sentry-agentic-secrets \
...
```
postgresql://sentry:sentry_secure_password_change_me@localhost:5432/sentry_soc
```
