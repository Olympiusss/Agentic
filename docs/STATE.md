# Where Sentry Agentic stores state
...
**Authoritative store: `~/.sentry-agentic/secrets.enc`** (Fernet-encrypted JSON,
chmod 600). The symmetric key lives at `~/.sentry-agentic/master.key` (chmod 600,
...
1. `EncryptedFileBackend` — `~/.sentry-agentic/secrets.enc` (preferred)
2. `EnvironmentBackend` — `os.environ`
3. `DotEnvBackend` — `~/.sentry/.env` (**legacy, prefer migrate**)
...
**Migrating legacy secrets.** Run once:
    ...
    python scripts/migrate_secrets.py --purge  # also strip from ~/.sentry/.env

**Backup.** Back up `~/.sentry-agentic/` as a unit — both files are needed.
...
moves values from here into `~/.sentry-agentic/secrets.enc`; run with `--purge`
to clear it.

## `~/.sentry/general_config.json`
...
own internal state for provider config and cache. Sentry Agentic does **not**
...
MemPalace is Sentry Agentic's cross-session memory layer — agents write IOCs,
...
**Palace location: `~/.sentry-agentic/mempalace/palace`.** Override with
...
```
~/.sentry-agentic/mempalace/palace/
...
Does *not* survive `rm -rf ~/.sentry-agentic/`.

**Backup.** Tar the directory as a unit:

    tar -czf mempalace-backup-$(date +%Y%m%d).tar.gz ~/.sentry-agentic/mempalace/
...
once:
    ...
    mv ~/.mempalace ~/.sentry-agentic/mempalace
...
- `sentry-postgres` — Postgres data.
- `sentry-redis` — Redis (ARQ queue + rate limiting).
- `sentry-bifrost` — Bifrost's internal state (if any persistent).

## Quick reference

| Thing | Where |
|---|---|
| API keys, tokens | `~/.sentry-agentic/secrets.enc` |
...
| Logs, PIDs | `logs/` |
| MemPalace palace (agent memory) | `~/.sentry-agentic/mempalace/palace/` |
| Legacy secrets (migrate from) | `~/.sentry/.env` |
