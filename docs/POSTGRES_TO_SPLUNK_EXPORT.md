...
3. **Create a New HEC Token**
   - Click **New Token**
   - Give it a name (e.g., "Sentry Agentic Export")
   - Select source type: `json` or create custom source types like `sentry:finding` and `sentry:case`
   - Select the target index (e.g., `main` or create a dedicated index like `sentry`)
   - Click **Review** → **Submit**
...
```spl
index=sentry source="postgresql_export"
```

### View Only Findings

```spl
index=sentry sourcetype="sentry:finding"
```

### View Only Cases

```spl
index=sentry sourcetype="sentry:case"
```

### High Severity Findings

```spl
index=sentry sourcetype="sentry:finding" severity="high" OR severity="critical"
...
```spl
index=sentry sourcetype="sentry:finding" anomaly_score>0.8
...
| join finding_id [
    search index=sentry sourcetype="sentry:finding"
...
- Run: `psql -d sentry_soc -c "SELECT COUNT(*) FROM findings;"`
...
3. **Index**: Create a dedicated index for Sentry Agentic data to improve search performance
...
# index=sentry | stats count by sourcetype
