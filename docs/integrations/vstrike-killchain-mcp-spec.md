...
MCP tool that lets Sentry Agentic drive kill-chain replays through the VStrike UI.

The Sentry Agentic side of the wire is already shipped. Once VStrike's MCP server
exposes `ui-killchain-replay`, Sentry Agentic's "Play" button (visible in the iframe
toolbar inside every Sentry Agentic case dialog) starts working with no further code
changes on Sentry Agentic's side. Until then, the button surfaces a friendly
"VStrike server doesn't yet implement kill-chain replay" notice.

## Why we need this

When a Sentry Agentic analyst opens a case enriched by VStrike, every finding's
...
Sentry Agentic walks the case's findings, dedupes by asset, and ships VStrike a
...
- **Returns a `content` text confirmation.** Same shape as the existing
  ...
  `isError` as the failure signal so Sentry Agentic's existing extractor works
  unchanged.
...
1. **Edge animations.** Does the VStrike UI already render edge animations
   ...
   wants. Sentry Agentic's UX assumption is that VStrike draws the edge into each
...
2. **Playback-finished event.** Can the iframe send a "playback finished"
   message back over its existing WebSocket so Sentry Agentic can sync its
   EventTimeline scrubber? Bidirectional channel — nice-to-have, not a
   blocker. If you can emit this, Sentry Agentic will hook it up; if not, we'll
   add a dwell-sum estimate on Sentry Agentic's side.

3. **`node_id` format.** Sentry Agentic's findings carry asset identifiers that
   we believe match VStrike's `node-list` output (the `id` / `network_id`
   / `uuid` field, in that priority order). Please confirm. If `node_id`
   needs to be the same shape `node-list` returns per network, Sentry Agentic
   already produces that shape.

## Curl example — what Sentry Agentic sends

This is what a real call looks like once the tool ships. Sentry Agentic already
...
Direct questions about the Sentry Agentic-side implementation to:
- Sentry Agentic repo: <https://github.com/Sentry Agentic-SOC/sentry-agentic>
