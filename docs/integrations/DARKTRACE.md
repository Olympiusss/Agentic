...
Sentry Agentic accepts **pushed alerts** from Darktrace via signed HTTP webhooks. Both
...
Each alert is transformed into a Sentry Agentic `finding` (`data_source = "darktrace"`)
...
2. **URL**: `https://<sentry-agentic-host>:6987/api/webhooks/darktrace/model-breach` (repeat for `ai-analyst` and `system-status` — each endpoint is separate so you can wire each stream independently).
3. **Authentication**: select **HMAC-SHA256**. Paste the same value you used for `DARKTRACE_WEBHOOK_SECRET`.
4. **Signature header**: `X-Darktrace-Signature` — hex HMAC-SHA256 over the raw request body. Sentry Agentic also accepts the `sha256=...` prefix style.
5. Click **Test** — Sentry Agentic returns `202 Accepted` on success, `401` if the signature doesn't match.

### SaaS vs on-prem notes

- **SaaS** tenants cannot emit syslog over the public internet — webhooks are the only option. Make sure Sentry Agentic is reachable at a TLS-terminated, publicly-routable URL.
- **On-prem** masters work the same way; if your Sentry Agentic instance is only reachable internally, point Darktrace at the internal hostname. Syslog remains possible for on-prem deployments but is not used by this integration.

## Verifying end-to-end

1. Run Sentry Agentic's backend (`./start_web.sh` or `uvicorn backend.main:app --reload`).
2. Confirm the receiver is live:
   ```bash
   curl https://<sentry-agentic-host>:6987/api/webhooks/darktrace/health
   ```
3. Smoke test with a signed payload:
   ...
   curl -X POST https://<sentry-agentic-host>:6987/api/webhooks/darktrace/model-breach \
     -H "Content-Type: application/json" \
     -H "X-Darktrace-Signature: $SIG" \
     -d "$BODY"
   ```
   Expect `HTTP 202` with `{"accepted": true, "finding_id": "f-YYYYMMDD-xxxxxxxx"}`.
4. Confirm ingestion: `curl https://<sentry-agentic-host>:6987/api/findings?data_source=darktrace`.

## Idempotency

Sentry Agentic derives each `finding_id` from the alert's stable identifier
