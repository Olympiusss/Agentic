# Sentry Agentic Helm chart — secret management patterns

The chart supports four ways to provide secrets to Sentry Agentic pods. Pick whichever
...
1. Write a plain Secret manifest with all the keys you need:
   ...
   kind: Secret
   metadata:
     name: sentry-agentic-secrets
     namespace: sentry-agentic
   type: Opaque
   stringData:
...
2. Seal it:
   ...
   ```bash
   kubeseal -o yaml < secret.yaml > sentry-agentic-sealed-secret.yaml
   ```
...
3. Apply the sealed manifest and install the chart:
   ...
   ```bash
   kubectl apply -f sentry-agentic-sealed-secret.yaml
   # The controller creates the sentry-agentic-secrets Secret automatically.

   helm install sentry-agentic ./helm/sentry-agentic \
     -n sentry-agentic --create-namespace \
     --set secrets.existingSecret=sentry-agentic-secrets
   ```
...
4. To rotate: re-run step 1 with new values, re-seal, re-apply. The chart
   ...
   Secret changes, trigger a rollout manually: `kubectl rollout restart -n sentry-agentic deploy`).
...
age-keygen -o ~/.sops/sentry-agentic.txt
...
1. Write the plain Secret manifest:
   ...
   ```yaml
   # secrets/sentry-agentic-secrets.yaml
   apiVersion: v1
   kind: Secret
   metadata:
     name: sentry-agentic-secrets
...
2. Encrypt in place:
   ...
   ```bash
   sops -e -i secrets/sentry-agentic-secrets.yaml
   ```
...
3. Deploy:
   ...
   export SOPS_AGE_KEY_FILE=~/.sops/sentry-agentic.txt
   sops -d secrets/sentry-agentic-secrets.yaml | kubectl apply -f -
...
4. Rotation: edit the encrypted file directly with `sops secrets/sentry-agentic-secrets.yaml`
...
sops -e -i helm/sentry-agentic/values-prod-secrets.yaml

# Install
helm secrets install sentry-agentic ./helm/sentry-agentic \
  -n sentry-agentic --create-namespace \
  -f helm/sentry-agentic/values-prod-secrets.yaml
```
...
All three work cleanly with Sentry Agentic's `secrets.existingSecret`. Never mix —
pick one and stick with it for the full Sentry Agentic install.
