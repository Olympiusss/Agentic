...
Stream security findings into Sentry Agentic from Apache Kafka topics. Runs
...
- You already publish security events to Kafka (Splunk HEC forwarders, Falco, custom
  producers) and want Sentry Agentic to consume them directly, rather than polling the source.
...
or [GH issue #83](https://github.com/Sentry Agentic-SOC/sentry-agentic/issues/83) for the
post-MVP roadmap.
...
Sentry Agentic ships a single-broker KRaft-mode Kafka service under the `kafka`
...
`sentry-network` Docker network). Data is persisted in the
...
```bash
docker exec sentry-kafka /opt/kafka/bin/kafka-topics.sh \
  --bootstrap-server localhost:9092 \
  --create --if-not-exists --topic security.findings \
  --partitions 1 --replication-factor 1
```

### 3. Enable Kafka in Sentry Agentic

Two equivalent paths — pick whichever you prefer.

**Option A: Settings UI** (recommended)

1. Open Sentry Agentic → **Settings** → **Kafka**.
...
curl -X PUT http://localhost:6987/api/kafka/config \
  ...
  -d '{
    ...
    "consumer_group": "sentry-agentic-soc",
...
- **Direct DB check**:
  ...
  ```bash
  docker exec -e PGPASSWORD=$POSTGRES_PASSWORD sentry-postgres \
    psql -U sentry -d sentry_soc \
...
(`sentry-agentic:dedup:kafka`). A duplicate within the retention window (24h
...
| `KAFKA_CONSUMER_GROUP` | `sentry-agentic-soc` | Consumer group id |
...
- Verify broker reachability: `docker exec sentry-daemon nc -z kafka 29092`
