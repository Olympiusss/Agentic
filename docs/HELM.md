# Deploying Sentry Agentic on Kubernetes

Sentry Agentic ships with a Helm chart under [`helm/sentry-agentic/`](../helm/sentry-agentic/) that
...
- A working container registry pull path. Release images are published to
  `ghcr.io/sentry-agentic-soc/sentry-agentic-backend` and `ghcr.io/sentry-agentic-soc/sentry-agentic-daemon` by
...
helm install sentry-agentic ./helm/sentry-agentic \
  -f ./helm/sentry-agentic/values-dev.yaml \
  --namespace sentry-agentic --create-namespace \
  --set secrets.anthropicApiKey="$ANTHROPIC_API_KEY"
```
...
```bash
kubectl get pods -n sentry-agentic
kubectl get jobs -n sentry-agentic -l app.kubernetes.io/component=db-init

# End-to-end smoke test
helm test sentry-agentic -n sentry-agentic

# Port-forward and hit the API
kubectl port-forward -n sentry-agentic svc/sentry-agentic-backend 6987:6987
...
```yaml
secrets:
  existingSecret: sentry-agentic-prod-secrets
```
...
postgresql:
  ...
  external:
    ...
    port: 5432
    database: sentry-agentic
    username: sentry-agentic
    existingSecret: sentry-agentic-db-credentials
...
ingress:
  ...
  hosts:
    - host: sentry-agentic.example.com
      paths:
        - path: /
          pathType: Prefix
  tls:
    - secretName: sentry-agentic-tls
      hosts:
        - sentry-agentic.example.com
```
...
2. Creates a `_sentry-agentic_schema_versions` marker table
...
`helm/sentry-agentic/files/database-init/` because Helm can only read from inside the
...
```bash
cp database/init/NEW.sql helm/sentry-agentic/files/database-init/
# Then edit helm/sentry-agentic/values.yaml and add "NEW.sql" to dbInit.sqlFiles in
...
```bash
helm upgrade sentry-agentic ./helm/sentry-agentic -n sentry-agentic --reuse-values --wait
```
...
> helm upgrade sentry-agentic ./helm/sentry-agentic -n sentry-agentic --reset-then-reuse-values --wait
>
> # Or pass an explicit values file so the new defaults aren't lost
> helm upgrade sentry-agentic ./helm/sentry-agentic -n sentry-agentic -f my-values.yaml --wait
> ```
>
...
```bash
helm uninstall sentry-agentic -n sentry-agentic
kubectl delete pvc -n sentry-agentic -l app.kubernetes.io/instance=sentry-agentic  # optional: drop data
kubectl delete namespace sentry-agentic
```
...
helm dependency update helm/sentry-agentic
```
...
Recommended for Sentry Agentic workloads: LLM calls are I/O-bound, so CPU is a poor
...
3`); if it still fails, check `kubectl logs -n sentry-agentic job/sentry-agentic-db-init` and
the Postgres pod logs.

**Daemon pod keeps restarting** — probe the `/health` endpoint directly:
`kubectl exec -n sentry-agentic sentry-agentic-daemon-0 -- curl http://localhost:9091/health`.
