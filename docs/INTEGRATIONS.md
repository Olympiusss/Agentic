...
|----------|---------|--------|
| Core | sentry-findings, approval, attack-layer, tempo-flow | Implemented |
...
- **Bi-directional Sync**: Case status changes in Sentry Agentic sync back to Elastic Security alerts
...
Open-source Cuckoo fork for on-prem detonation. Sentry Agentic ships an MCP client
(`tools/cape_sandbox.py`) that talks to an existing CAPE deployment over
its REST API — Sentry Agentic does **not** host CAPE itself. CAPE requires KVM and
...
- No binary bytes are ever sent from Sentry Agentic. Only hash-cache lookups and
...
Closes the loop by letting Sentry Agentic propose and (with approval) execute
...
Benefits:
- Normalize log formats before Sentry Agentic analysis
- Filter noise, reduce Splunk ingestion 30-50%
- Enrich events with GeoIP, asset info
- Route data to multiple destinations

```
Data Sources -> Cribl Stream -> Sentry Agentic LogLM
                            -> Splunk
                            -> S3/Data Lake
```
...
VStrike enriches Sentry Agentic findings with network topology, asset, segment,
and mission-system context, then pushes the enriched findings back into
Sentry Agentic. Sentry Agentic can also query VStrike on demand for asset topology and blast
radius.
...
|-----------|-----------------|---------|
| VStrike → Sentry Agentic | `POST /api/integrations/vstrike/findings` | Push enriched findings (batched) |
| Sentry Agentic → VStrike | `GET /api/integrations/vstrike/health` | Outbound reachability check |
| Sentry Agentic → VStrike | `GET /api/integrations/vstrike/topology/asset/{id}` | Asset topology lookup |
| Sentry Agentic → VStrike | `GET /api/integrations/vstrike/topology/asset/{id}/adjacent` | One-hop neighbors |
| Sentry Agentic → VStrike | `GET /api/integrations/vstrike/topology/asset/{id}/blast-radius` | Blast radius |
...
VSTRIKE_INBOUND_API_KEY="<bearer token Sentry Agentic expects on inbound push>"
```
