...
Complete guide to deploying Sentry Agentic to VMs and managing production environments.
...
Sentry Agentic can be deployed in several configurations:
...
```bash
# As deployer user
sudo mkdir -p /opt/sentry-agentic
sudo chown deployer:deployer /opt/sentry-agentic
cd /opt/sentry-agentic

# Clone repository
git clone https://github.com/your-org/sentry-agentic.git .
...
export IMAGE_NAME=your-org/sentry-agentic
...
encrypted at `~/.sentry-agentic/secrets.enc`. For server-side / orchestrated
...
```bash
# Database
DATABASE_URL=postgresql://sentry:secure_password@postgres:5432/sentry_soc
...
**File**: `/opt/sentry-agentic/docker-compose.yml`
...
services:
  ...
  backend:
    ...
    volumes:
      ...
      - sentry-agentic_secrets:/root/.sentry-agentic
    ports:
      - "6987:6987"
    depends_on:
      - postgres
...
volumes:
  postgres_data:
  sentry-agentic_secrets:
```

### Nginx Reverse Proxy (Optional)

**File**: `/etc/nginx/sites-available/sentry-agentic`

```nginx
server {
    listen 80;
    server_name app.sentry.ai;
...
server {
    ...
    # SSL certificates
    ssl_certificate /etc/letsencrypt/live/app.sentry.ai/fullchain.pem;
    ssl_certificate_key /etc/letsencrypt/live/app.sentry.ai/privkey.pem;
    ...
    # Frontend
    location / {
        root /opt/sentry-agentic/frontend/build;
...
# Import Sentry Agentic dashboard
```
...
vi /opt/sentry-agentic/scripts/backup.sh
```

```bash
#!/bin/bash
BACKUP_DIR="/opt/sentry-agentic/backups"
DATE=$(date +%Y%m%d_%H%M%S)
BACKUP_FILE="$BACKUP_DIR/sentry_$DATE.sql"

# Create backup
docker-compose exec -T postgres pg_dump -U sentry sentry_soc > $BACKUP_FILE
...
0 2 * * * /opt/sentry-agentic/scripts/backup.sh >> /opt/sentry-agentic/logs/backup.log 2>&1
```
...
# Restore from backup
gunzip -c backups/sentry_20260127.sql.gz | docker-compose exec -T postgres psql -U sentry sentry_soc
...
# Test connection
docker-compose exec postgres psql -U sentry -d sentry_soc -c "SELECT 1;"

# Check network
docker network ls
docker network inspect sentry-agentic_default
```
...
# Remove old images
docker images | grep sentry-agentic | grep -v latest | awk '{print $3}' | xargs docker rmi

# Cleanup old logs
find /opt/sentry-agentic/logs -name "*.log" -mtime +7 -delete
```
...
gunzip -c backups/pre-v1.2.3.sql.gz | docker-compose exec -T postgres psql -U sentry sentry_soc
