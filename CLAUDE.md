...
├── sentry-core/       # Git submodule: core AI/detection library
├── database/
...
encrypted at `~/.sentry-agentic/secrets.enc` — see [docs/STATE.md](docs/STATE.md).
...
chart bundles a *copy* under `helm/sentry-agentic/files/database-init/` (Helm can
only read files inside the chart directory). You must (1) copy the file
into the chart bundle, (2) add it to `helm/sentry-agentic/values.yaml`
...
filename already has a row in `_sentry-agentic_schema_versions`, in which case
...
|-----------|------|---------|
| `sentry-core` | `./sentry-core` | Core AI and detection library |
| `mcp-servers` | `./mcp-servers` | MCP server implementations |

Both are installed as editable packages (`-e ./sentry-core`, `-e ./mcp-servers`) in `requirements.txt`. If submodules aren't initialized, `start_web.sh` skips their installation gracefully.
