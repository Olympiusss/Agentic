...
and two of them disagreed:
  ...
  mcp-config.json             → ~/.sentry-agentic/mempalace/palace
...
_DEFAULT_PALACE = Path.home() / ".sentry-agentic" / "mempalace" / "palace"
...
def get_palace_path(*, ensure_exists: bool = True) -> Path:
    ...
    to ``~/.sentry-agentic/mempalace/palace``. When ``ensure_exists=True``
