"""Prompt-injection defenses for the Sentry Agentic LLM call path (issue #87).
...
_TOOL_RESULT_OPEN = '<sentry-agentic:tool_result source="{source}" tool="{tool}">'
_TOOL_RESULT_CLOSE = "</sentry-agentic:tool_result>"
...
def wrap_tool_result(
    ...
    """Wrap *content* in a `<sentry-agentic:tool_result>` block.
    ...
    if not isinstance(content, str):
        content = str(content)
    if content.startswith("<sentry-agentic:tool_result"):
        return content
