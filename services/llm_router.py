...
Per GH #84 PR-B, Sentry Agentic routes **all** LLM traffic through Bifrost —
...
class LLMRouter:
    ...
    async def dispatch(
        ...
        *,
        ...
        ``x-bf-lh-sentry-agentic-interaction-id`` header — Bifrost's logging plugin
        captures any ``x-bf-lh-*`` header into ``LogEntry.metadata``, so
        operators can correlate Sentry Agentic's local ``LLMInteractionLog`` row
        ...
        if interaction_id:
            extra_headers["x-bf-lh-sentry-agentic-interaction-id"] = interaction_id
        try:
