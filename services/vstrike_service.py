...
VStrike pushes enriched findings to Sentry Agentic, but we also query it for asset
...
def get_vstrike_service() -> Optional[VStrikeService]:
    ...
    Sentry Agentic's secrets manager (encrypted store → env → dotenv → keyring,
    ...
    Sentry Agentic now exchanges username + password for a JWT internally on first
