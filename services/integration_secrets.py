...
Sentry Agentic's persistence story for integration credentials is split:
...
- **Non-secret config** (URLs, regions, verify_ssl flags, paths) goes into the
  ...
  mirrored to ``~/.sentry/integrations_config.json`` for back-compat.
- **Secret credentials** (API keys, passwords, bearer tokens) go into the
  encrypted secrets store at ``~/.sentry-agentic/secrets.enc`` via
