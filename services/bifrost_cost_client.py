...
Bifrost is the authoritative cost source for every LLM call Sentry Agentic makes —
...
def search_logs(
    *,
    ...
    ``x-bf-lh-sentry-agentic-interaction-id`` header) is not currently exposed
    as a query param — callers that need correlation by Sentry Agentic's
...
def get_vk_quota(vk: str) -> Optional[Dict[str, Any]]:
    ...
    to render "$X of $Y consumed" without round-tripping through Sentry Agentic's
    aggregations.
    """
    if not vk:
        return None
