...
try:
    ...
    _cs_tracer = get_tracer("sentry-agentic.services.claude")
    _cs_meter = get_meter("sentry-agentic.services.claude")
...
class ClaudeService:
    ...
    def _get_default_system_prompt(self) -> str:
        ...
        return """You are Claude, an AI assistant for security operations and analysis in the Sentry Agentic platform.
...
1. **Findings & Cases**: Retrieve and analyze security findings and cases from Sentry Agentic
   - Finding IDs start with "f-" (e.g., "f-20260109-40d9379b")
   - Case IDs start with "case-" (e.g., "case-20260114-a1b2c3d4")
   - Use sentry-findings server tools: list_findings, get_finding, list_cases, get_case, create_case, update_case
...
- Finding IDs: "f-YYYYMMDD-XXXXXXXX" → Use sentry-findings_get_finding tool
- Case IDs: "case-YYYYMMDD-XXXXXXXX" → Use sentry-findings_get_case tool  
...
Provide a structured summary preserving all critical context."""
                ...
                if server_name:
                    try:
                        ...
                        if mcp_client:
                            ...
                            # block in <sentry-agentic:tool_result> so attacker payloads
    ...
    def chat(
        ...
        try:
            ...
            # #185: tag the upstream Bifrost call with a Sentry Agentic interaction
            ...
            api_kwargs["extra_headers"] = {
                **_existing_extra,
                "x-bf-lh-sentry-agentic-interaction-id": _interaction_id,
            }
                    ...
                    for tool_round in range(5):
                        ...
                        api_kwargs["extra_headers"] = {
                            **(api_kwargs.get("extra_headers") or {}),
                            "x-bf-lh-sentry-agentic-interaction-id": _round_interaction_id,
                        }
    ...
    async def chat_stream(
        ...
        try:
            ...
            while iteration < max_iterations:
                ...
                api_kwargs["extra_headers"] = {
                    **(api_kwargs.get("extra_headers") or {}),
                    "x-bf-lh-sentry-agentic-interaction-id": _stream_interaction_id,
                }
