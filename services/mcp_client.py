...
class MCPClient:
    ...
    def _missing_credentials_for(self, server) -> List[str]:
        ...
        Checks both ``os.environ`` and the Sentry Agentic secrets manager, so a
    ...
    async def call_tool(self, server_name: str, tool_name: str, arguments: Dict[str, Any], timeout: float = 30.0) -> Dict[str, Any]:
        ...
        try:
            ...
            _mcp_tracer = get_tracer("sentry-agentic.services.mcp_client")
            _mcp_span = _mcp_tracer.start_span(
                ...
                attributes={
                    ...
                    "sentry-agentic.tool.input_size": len(_json.dumps(arguments, default=str)),
                },
            )
        except Exception:
            _SC = None
            ...
            if not await self.connect_to_server(server_name, persistent=True):
                ...
                try:
                    if _mcp_span is not None:
                        _mcp_span.set_attribute("sentry-agentic.tool.success", False)
                        _mcp_span.end()
                except Exception:
                    pass
                return _err
        ...
        try:
            ...
            try:
                if _mcp_span is not None:
                    ...
                    _mcp_span.set_attribute("sentry-agentic.tool.success", not is_err)
                    _mcp_span.set_attribute("sentry-agentic.tool.output_size", len(_json.dumps(result, default=str)))
                    _mcp_span.set_attribute("sentry-agentic.tool.duration_ms", round((_time.monotonic() - _mcp_t0) * 1000, 1))
                    _mcp_span.end()
            except Exception:
                pass
            return result
