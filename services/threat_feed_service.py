...
def parse_stix_indicator(
    ...
    for sentry-agentic_type, value in _extract_observables(pattern):
        out.append(
            NormalizedIndicator(
                indicator_type=sentry-agentic_type,
...
def _extract_observables(pattern: str) -> Iterable[Tuple[str, str]]:
    """Best-effort extraction of (sentry-agentic_type, value) pairs from a STIX 2.1 pattern.
    ...
    for clause in [c.strip() for c in body.split(" OR ")]:
        ...
        if not value:
            continue
        sentry-agentic_type = _STIX_TO_VIGIL_TYPE.get(key)
        if not sentry-agentic_type:
            logger.debug("Skipping unrecognized STIX pattern key: %s", key)
            continue
        yield sentry-agentic_type, value
