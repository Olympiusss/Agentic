...
chart reads from a **copy** under `helm/sentry-agentic/files/database-init/`
...
- **Helm chart** — the `db-init` Job iterates over
  `helm/sentry-agentic/values.yaml`'s `dbInit.sqlFiles` list in the **order
  written there**. Filename prefixes are decorative for this path; the
  list is authoritative. Files in `helm/sentry-agentic/files/database-init/`
...
1. **Copy the file into the chart bundle**:
   ```bash
   cp database/init/NEWFILE.sql helm/sentry-agentic/files/database-init/
   ```
   The `Helm Chart / Lint and Template` workflow runs
   `diff -r database/init helm/sentry-agentic/files/database-init` on every PR and
   will fail if these two directories drift.

2. **Add the filename to `helm/sentry-agentic/values.yaml`** under
...
3. **Verify with `helm template`** that the rendered dbInit Job script
   ...
   ```bash
   helm template release-check helm/sentry-agentic \
...
Earlier versions of `helm/sentry-agentic/values.yaml` listed these as ghost
...
`_sentry-agentic_schema_versions` to mark it "applied." So every v0.1.x Helm
...
the Job will check `_sentry-agentic_schema_versions`, see the ghost row, and
...
Same drill — copy the updated file to `helm/sentry-agentic/files/database-init/`
...
- [`helm/sentry-agentic/files/database-init/README.md`](../../helm/sentry-agentic/files/database-init/README.md)
