...
VStrike is a network-topology fusion layer that enriches Sentry Agentic LogLM
...
to Sentry Agentic via `POST /api/integrations/vstrike/findings`.
...
class VStrikeFinding(BaseModel):
    ...
    `finding_id` must match a Sentry Agentic LogLM finding already known to Sentry Agentic
