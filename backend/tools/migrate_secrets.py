"""CLI: move secrets from ~/.sentry/.env into ~/.sentry-agentic/secrets.enc.
...
def main(argv: Optional[List[str]] = None) -> int:
    parser = argparse.ArgumentParser(
        ...
        description=(
            "Move secrets from the dotenv backend (~/.sentry/.env) into "
            "the encrypted backend (~/.sentry-agentic/secrets.enc)."
        ),
    )
    parser.add_argument(
        "--keys",
