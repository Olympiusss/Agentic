...
async def get_s3_config():
    ...
    try:
        ...
        config_file = Path.home() / ".sentry" / "s3_config.json"
        if config_file.exists():
            with open(config_file, "r") as f:
                config = json.load(f)
                return {
...
async def get_theme_config():
    ...
    try:
        ...
        # Fallback to file-based config
        config_file = Path.home() / ".sentry" / "theme_config.json"
        if config_file.exists():
            with open(config_file, "r") as f:
                config = json.load(f)
...
async def get_integrations_config():
    ...
    try:
        ...
        # Fallback to file-based config
        config_file = Path.home() / ".sentry" / "integrations_config.json"
        if config_file.exists():
            with open(config_file, "r") as f:
                config = json.load(f)
                redacted = {
...
async def secrets_status() -> Dict[str, Any]:
    ...
    creds not landing in ~/.sentry-agentic/secrets.enc?". Includes the chosen
...
async def secrets_migrate_to_encrypted(
    ...
    """Move secrets from ``~/.sentry/.env`` to ``~/.sentry-agentic/secrets.enc``.
