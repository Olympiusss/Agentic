"""
Secrets Manager for Sentry Agentic

Provides pluggable secrets storage backends with priority fallback:
1. Encrypted local file at ``~/.sentry-agentic/secrets.enc`` (preferred; at-rest encrypted)
...
class DotEnvBackend(SecretsBackend):
    ...
    def __init__(self, env_file: Optional[Path] = None):
        ...
        self.env_file = env_file or Path.home() / ".sentry" / ".env"
    ...
    def set(self, key: str, value: str) -> bool:
        ...
        try:
            ...
            with open(self.env_file, "w") as f:
                f.write("# Sentry Agentic Secrets\n")
                f.write(
...
class EncryptedFileBackend(SecretsBackend):
    ...
    Secrets live in a Fernet-encrypted JSON blob at ``~/.sentry-agentic/secrets.enc``.
    The symmetric key lives alongside it in ``~/.sentry-agentic/master.key`` (chmod
    ...
    DEFAULT_DIR = Path.home() / ".sentry-agentic"
    ...
    def _load_or_create_master_key(self) -> bytes:
        ...
        logger.info(f"Generated new Sentry Agentic master key at {self.master_key_path}")
        return key
    ...
    def _load_cache(self) -> Dict[str, str]:
        ...
        try:
            ...
            logger.error(
                ...
                f"~/.sentry-agentic/master.key from a backup."
            )
            self._cache = {}
            self._cache_mtime = current_mtime
        return self._cache
...
class SecretsManager:
    ...
    1. Encrypted local file (``~/.sentry-agentic/secrets.enc``; preferred)
    2. Environment variables
    3. .env file (legacy / interoperability)
    4. Keyring (only when explicitly enabled)
    ...
    def migrate_dotenv_secrets_to_encrypted(
        ...
        For each key currently stored in ``~/.sentry/.env`` (the dotenv
        backend's file):
            ...
            result["errors"].append(
                {
                    ...
                    "error": (
                        ...
                        "Install `cryptography` and ensure ~/.sentry-agentic/master.key "
                        "exists, then retry."
                    ),
                }
            )
        ...
        if enable_keyring is None:
            ...
            if enable_keyring_env in ("true", "1", "yes", "on"):
                ...
                try:
                    ...
                    config_file = Path.home() / ".sentry" / "general_config.json"
