...
```spl
# View findings only
index=main sourcetype="sentry:finding"

# View cases only  
index=main sourcetype="sentry:case"

# High severity findings
index=main sourcetype="sentry:finding" severity IN (high, critical)
...
| "No findings to export" | Check you have data: `psql -d sentry_soc -c "SELECT COUNT(*) FROM findings;"` |
...
```spl
index=main sourcetype="sentry:finding" | stats count by severity
```

**Case Status Timeline (Area Chart)**
```spl
index=main sourcetype="sentry:case" | timechart count by status
```
