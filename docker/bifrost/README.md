...
[Bifrost](https://github.com/maximhq/bifrost) is a compiled Go binary that exposes a unified LLM gateway — OpenAI-format requests on `/v1` plus provider-native passthroughs (e.g. `/anthropic`). Sentry Agentic routes **all** LLM traffic through Bifrost so caching, cost tracking, and budget enforcement live in one place.

## How Sentry Agentic uses Bifrost
...
Before shipping any change that modifies how Sentry Agentic talks to Bifrost, run:
...
`docker/bifrost/config.json` declares the providers, the models Bifrost will expose, and the cache backend. API keys are **not** written into the config file — they are injected as environment variables at container start time (`env.ANTHROPIC_API_KEY`, `env.OPENAI_API_KEY`, `env.OLLAMA_URL`). Sentry Agentic's backend reads per-provider keys from its own `secrets_manager`; what's in Bifrost's env are the fallback/default keys used when a provider row in `llm_provider_configs` doesn't override them.
...
local SQLite store (`./logs.db` inside the Bifrost container). Sentry Agentic's
...
**Per-call correlation:** Sentry Agentic attaches a custom header
`x-bf-lh-sentry-agentic-interaction-id: <uuid>` to every upstream call (the
...
Sentry Agentic benefits from two independent caching layers. They're **complementary**, not redundant:
...
Run `python scripts/bifrost_cache_status.py` to check whether it's live. Sentry Agentic's unified routing does **not** depend on this layer — it's optional cost gravy on top of Anthropic's native prefix caching.
...
"containerDefinitions": [
  {
    "name": "bifrost",
    "secrets": [
      {"name": "ANTHROPIC_API_KEY", "valueFrom": "arn:aws:secretsmanager:...:sentry-agentic/anthropic-api-key"},
      {"name": "OPENAI_API_KEY",    "valueFrom": "arn:aws:secretsmanager:...:sentry-agentic/openai-api-key"}
    ]
  }
]
```
...
envFrom:
  - secretRef:
      name: sentry-agentic-llm-provider-keys
```
