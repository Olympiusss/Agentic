#!/bin/bash
# Start Sentry Agentic in daemon mode (background) - Updated for v2.0

echo "=========================================="
echo "Sentry Agentic v2.0 - Background Mode"
echo "=========================================="
...
if [ -d ".git" ]; then
    if [ ! -f "sentry-core/setup.py" ] && [ ! -f "sentry-core/pyproject.toml" ]; then
...
if command -v docker &> /dev/null; then
    if docker ps --format '{{.Names}}' | grep -q "sentry-postgres"; then
        ...
        for i in {1..30}; do
            if docker exec sentry-postgres pg_isready -U sentry -d sentry_soc &> /dev/null 2>&1; then
    ...
    if docker ps --format '{{.Names}}' | grep -q "sentry-redis"; then
    ...
    if docker ps --format '{{.Names}}' | grep -q "sentry-bifrost"; then
