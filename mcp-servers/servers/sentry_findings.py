...
mcp = FastMCP("sentry-findings")

DATA_DIR = Path(os.environ.get("SENTRY_DATA_DIR", Path(__file__).parent.parent / "data"))
...
def get_data_service():
    ...
    if _data_service is None:
        from sentry_core.services import DatabaseDataService
        _data_service = DatabaseDataService()
        
        # Log which backend we're using
        backend_info = _data_service.get_backend_info()
        logger.info(f"MCP sentry-findings using backend: {backend_info['backend']}")
