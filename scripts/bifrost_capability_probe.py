...
Sentry Agentic now routes *all* LLM traffic through Bifrost, including Anthropic
...
async def probe_logging_metadata() -> bool:
    ...
    Sentry Agentic tags every Anthropic call with ``x-bf-lh-sentry-agentic-interaction-id`` so
    the dashboard can correlate Bifrost log rows back to a specific Sentry Agentic
    ...
    Strategy:
      1. Send a minimal Anthropic call carrying ``x-bf-lh-sentry-agentic-probe-id``
    ...
    Pass = the row exists AND ``sentry-agentic-probe-id`` round-tripped intact.
    """
    import httpx
    ...
    try:
        ...
        await client.messages.create(
            ...
            extra_headers={"x-bf-lh-sentry-agentic-probe-id": probe_id},
        )
    except Exception as exc:  # noqa: BLE001
    ...
    for row in rows:
        ...
        if meta.get("sentry-agentic-probe-id") == probe_id:
