#!/usr/bin/env python3
"""Migrate legacy secrets into ``~/.sentry-agentic/secrets.enc``.

Historically Sentry Agentic spread secrets across several places:

- ``.env`` at the repo root
- ``~/.sentry/.env`` (the ``DotEnvBackend`` default)
- OS env vars
- macOS Keychain (only if enabled)

This script reads every well-known secret name from all of those stores,
writes them to the new encrypted backend (``~/.sentry-agentic/secrets.enc``), and
then reports on what was moved. It does **not** delete from the old
stores by default — pass ``--purge`` to strip migrated values from
``~/.sentry/.env`` (the root ``.env`` is never touched automatically;
...
def main() -> int:
    ...
    parser.add_argument(
        "--purge",
        action="store_true",
        help="Delete migrated values from ~/.sentry/.env after success.",
    )
    ...
    dotenv = DotEnvBackend()  # default path: ~/.sentry/.env
