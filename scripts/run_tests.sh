...
if $DOCKER_COMPOSE exec -T postgres psql -U postgres -d sentry -c "SELECT 1;" > /dev/null 2>&1; then
  echo -e "${GREEN}✓ PASS${NC}"
  ((PASSED_TESTS++))
else
  echo -e "${RED}✗ FAIL${NC}"
...
TABLE_COUNT=$($DOCKER_COMPOSE exec -T postgres psql -U postgres -d sentry -t -c "SELECT COUNT(*) FROM information_schema.tables WHERE table_schema='public';" 2>/dev/null | tr -d ' ')
