...
def test_wrap_messages_rewrites_text_tool_result():
    ...
    assert "<sentry-agentic:tool_result" in inner and "raw splunk output" in inner
...
def test_wrap_messages_handles_string_inner_content():
    ...
    assert isinstance(wrapped, str) and wrapped.startswith("<sentry-agentic:tool_result")
...
async def test_dispatch_invokes_sanitize_hook(monkeypatch):
    ...
    assert "<sentry-agentic:tool_result" in inner
    # Attacker close tag must have been escaped — the only </sentry-agentic:tool_result>
    # is the wrapper's own.
    assert inner.count("</sentry-agentic:tool_result>") == 1
