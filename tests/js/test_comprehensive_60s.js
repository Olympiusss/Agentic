...
(async () => {
  ...
  try {
    ...
    const chatButton = await page.$('button[aria-label*="Sentry Agentic Chat"]');
    ...
    for (let i = 0; i < 12; i++) {
      ...
      if (/sentry|tempo-flow|approval|attack-layer/i.test(chatContent)) indicators.push('SERVERS');
    ...
    const hasServerNames = /sentry-findings|tempo-flow|approval|attack-layer/i.test(aiResponse);
    ...
    const serverMatches = aiResponse.match(/sentry-findings|tempo-flow|approval|attack-layer|github|crowdstrike|sentinelone/gi) || [];
