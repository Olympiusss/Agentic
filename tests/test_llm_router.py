...
async def test_dispatch_propagates_interaction_id_as_bifrost_log_header_openai():
    """#185: each LLM call carries an `x-bf-lh-sentry-agentic-interaction-id` header
    so Bifrost's logging plugin can correlate the LogEntry back to Sentry Agentic's
    ...
    assert headers.get("x-bf-lh-sentry-agentic-interaction-id") == interaction_id
