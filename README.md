...
Sentry Agentic is the open-source AI SOC where your playbooks are plain-text files, your agent logic is readable Python, and your integrations use an open standard ([MCP](https://modelcontextprotocol.io/)). Every proprietary AI SOC on the market is a black box you rent. Sentry Agentic is a capability you own.
...
Workflows are the operational core of Sentry Agentic. Each worfklow chains multiple specialized AI agents into an end-to-end playbook that executes with a single command. No manual hand-offs, no copy-pasting between tools — the agents coordinate automatically.  
...
Sentry Agentic uses the [Model Context Protocol](https://modelcontextprotocol.io/) to connect agents to your existing tools. These MCP servers give every agent real-time access to your SIEM, EDR, threat intel, sandbox, ticketing, and communication platforms — all through a unified interface.
...
| **Core** | Sentry Agentic Findings, Approval, ATT&CK Layer, Tempo Flow | Built-in SOC operations |
...
```bash
git clone --recurse-submodules https://github.com/Sentry Agentic-SOC/sentry-agentic.git
cd sentry-agentic
./start_web.sh
```
...
- An LLM provider key. Sentry Agentic supports Anthropic Claude (default), OpenAI, and Ollama (local) — configure providers in Settings → AI Config. See [`docker/bifrost/README.md`](docker/bifrost/README.md) for the multi-provider gateway. *(optional for initial testing)*
...
A production-style Helm chart lives at [`helm/sentry-agentic/`](helm/sentry-agentic/):

```bash
helm install sentry-agentic ./helm/sentry-agentic \
  --namespace sentry-agentic --create-namespace \
...
- **Auto-Contributor** — Automated competitive research against proprietary AI security platforms. Analyzes a vendor's capabilities, maps gaps versus Sentry Agentic and the open-source ecosystem, and generates ready-to-file GitHub issues with acceptance criteria. The goal: make Sentry Agentic a superset of every proprietary AI SOC, one contribution at a time. See [`contrib/auto-contributor/`](contrib/README.md)
...
**Find meaningful work automatically:** Sentry Agentic includes an [auto-contributor](contrib/README.md) tool that researches proprietary AI security platforms, identifies capability gaps, and generates ready-to-file GitHub issues. Pick a vendor, run the tool, and you'll have a scoped contribution spec in minutes.

**Join the community:** Connect with the Sentry Agentic community on [Discord](https://discord.gg/Kw68sPJU) to discuss ideas, get help, and collaborate with other contributors.
...
- [Sentry Agentic](https://sentry-agenticsoc.org/) — Project homepage
- [Sentry Agentic](https://sentry.ai) — Sentry Agentic sponsor; LogLM connects via MCP as an AI-native detection layer
