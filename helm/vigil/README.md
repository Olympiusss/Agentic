# Sentry Agentic — Helm chart

Production-style Helm chart for the Sentry Agentic platform. Ships the backend API
...
|---|---|---|---|
| `sentry-agentic-backend` | Deployment | 2 (default) | FastAPI API + bundled SPA on port 6987 |
| `sentry-agentic-daemon` | StatefulSet | **1 (singleton)** | Autonomous orchestrator; webhook=8081, metrics=9090, health=9091 |
| `sentry-agentic-llm-worker` | Deployment | 2 (default) | ARQ worker for Claude requests off Redis queue |
| `sentry-agentic-postgres` | StatefulSet | 1 | Opt-out via `postgresql.enabled=false` |
| `sentry-agentic-redis` | StatefulSet | 1 | Opt-out via `redis.enabled=false` |
| `sentry-agentic-db-init` | Job (Helm hook) | 1 per install/upgrade | Applies `database/init/*.sql` idempotently |
...
```yaml
secrets:
  existingSecret: sentry-agentic-secrets
```
...
```bash
helm upgrade sentry-agentic ./helm/sentry-agentic -n sentry-agentic --reuse-values
```
...
applied files in a `_sentry-agentic_schema_versions` table.
...
> helm upgrade sentry-agentic ./helm/sentry-agentic -n sentry-agentic --reset-then-reuse-values
>
> # Or pass an explicit values file so the new defaults aren't lost
> helm upgrade sentry-agentic ./helm/sentry-agentic -n sentry-agentic -f my-values.yaml
> ```
>
...
```bash
# Lint
helm lint helm/sentry-agentic
helm lint helm/sentry-agentic -f helm/sentry-agentic/values-dev.yaml

# Render without applying
helm template sentry-agentic helm/sentry-agentic \
  --set secrets.anthropicApiKey=test \
  --set secrets.postgresPassword=test

# Dry-run install
helm install --dry-run --debug sentry-agentic helm/sentry-agentic \
  --set secrets.anthropicApiKey=test \
  --set secrets.postgresPassword=test
```
...
`helm/sentry-agentic/files/database-init/`. CI (`.github/workflows/helm-chart.yml`) will
fail on drift. To sync after adding new init SQL:

```bash
cp database/init/*.sql helm/sentry-agentic/files/database-init/
