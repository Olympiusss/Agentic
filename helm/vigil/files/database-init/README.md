...
1. Copy it here: `cp database/init/NEWFILE.sql helm/sentry-agentic/files/database-init/`
...
- **Filename listed in `dbInit.sqlFiles` but missing from this directory**
  ...
  — *unless* the filename already has a row in `_sentry-agentic_schema_versions`,
...
`diff -r database/init helm/sentry-agentic/files/database-init` and fails on
