        ...
        if name == "cf_waf_block_ip":
            return _result(
                await asyncio.to_thread(
                    ...
                    ip=args.get("ip"),
                    reason=args.get("reason", "Sentry Agentic block"),
        ...
        if name == "cf_access_revoke_session":
            return _result(
                await asyncio.to_thread(
                    ...
                    reason=args.get("reason", "Sentry Agentic revoke"),
                )
            )
...
def _gateway_block_domain(
    ...
    payload = {
        "name": rule_name or f"sentry-agentic-block-{domain}",
...
def _lookup_domain_threat(
    ...
    return {
        ...
        "note": (
            ...
            "currently surfaces gateway categories metadata. Pair with Sentry Agentic's IP "
