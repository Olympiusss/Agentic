# Releasing Sentry Agentic

How Sentry Agentic is versioned and released.

## Versioning Policy

Sentry Agentic follows [Semantic Versioning](https://semver.org/).
...
- **`1.0.0` and later** — Stable. Breaking changes require a major bump.
  Sentry Agentic ships `1.0.0` once the agent and workflow schemas are considered
...
| `helm/sentry-agentic/Chart.yaml`       | `appVersion`                       | yes                           |
| `helm/sentry-agentic/Chart.yaml`       | `version`                          | yes (lockstep with appVersion)|
...
- **`appVersion`** — the Sentry Agentic release the chart deploys.
- **`version`** — the chart packaging version. Helm clients, `helm package`
  tarball names (`sentry-agentic-X.Y.Z.tgz`), Helm's local cache, and ArtifactHub all
...
template hotfix with no app code change), edit `helm/sentry-agentic/Chart.yaml`'s
...
1. Open a PR bumping the version to the new release (e.g. `0.2.0`) in
   ...
   - `VERSION`
   - `helm/sentry-agentic/Chart.yaml` `appVersion`
   - `helm/sentry-agentic/Chart.yaml` `version` (lockstep with `appVersion`)
...
5. **Manually create the GitHub Release** at
   [github.com/Sentry Agentic-SOC/sentry-agentic/releases/new](https://github.com/Sentry Agentic-SOC/sentry-agentic/releases/new),
