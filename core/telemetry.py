"""
core/telemetry.py — OpenTelemetry bootstrap for Sentry Agentic.
...
_investigation_id_var: ContextVar[Optional[str]] = ContextVar(
    "sentry-agentic_investigation_id", default=None
)
    ...
    resource = Resource.create(
        {
            ...
            "sentry-agentic.component": service_name,
        }
    )
...
def _install_json_logging() -> None:
    ...
    class _OTELJsonFormatter(logging.Formatter):
        def format(self, record: logging.LogRecord) -> str:
            ...
            if inv_id:
                entry["sentry-agentic.investigation.id"] = inv_id

            if record.exc_info:
                entry["exception"] = self.formatException(record.exc_info)

            for key, val in record.__dict__.items():
                if key.startswith("sentry-agentic."):
                    entry[key] = val
    ...
    try:
        log_dir = Path.home() / ".sentry"
        log_dir.mkdir(parents=True, exist_ok=True)
        file_handler = logging.FileHandler(log_dir / "sentry-agentic.log")
